...
(() => {
  ...
  // ---------- Liten guard: vent til Sync er klar ----------
  function whenSyncReady(fn){
    if (window.Sync) return fn();
    const t = setInterval(() => {
      if (window.Sync){ clearInterval(t); fn(); }
    }, 50);
    setTimeout(()=>clearInterval(t), 3000); // failsafe
  }
  ...
  async function autoSave(){
    try{
      await window.Sync.saveAddresses(ADDR); // bevarer ord
      markDirty(false);
    }catch(e){
      console.error(e);
  ...
  async function saveAllFromDOM(){
    ...
    try{
      await window.Sync.saveAddresses(ADDR);
      markDirty(false);
      alert('Lagret ✅');
    }catch(e){
  ...
  window.addEventListener('hashchange', ()=>{
    if (location.hash==='#admin') whenSyncReady(()=>load());
  });
