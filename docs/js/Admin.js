// js/Admin.js — config + address register + navShortcuts edit
(function(){
  'use strict';
  ...
  function adminLoad(){
    ...
    const fill = (key)=>{
      const sc = ns[key] || {};
      $('#ns_'+key+'_name') && ($('#ns_'+key+'_name').value = sc.name || '');
      $('#ns_'+key+'_query')&& ($('#ns_'+key+'_query').value= sc.query|| '');
      $('#ns_'+key+'_lat')  && ($('#ns_'+key+'_lat').value  = (typeof sc.lat==='number'? sc.lat : ''));
      $('#ns_'+key+'_lon')  && ($('#ns_'+key+'_lon').value  = (typeof sc.lon==='number'? sc.lon : ''));
    };
  ...
  function adminSave(){
    ...
    const take = (key)=>{
      const name  = ($('#ns_'+key+'_name')?.value || '').trim();
      const query = ($('#ns_'+key+'_query')?.value|| '').trim();
      const latRaw= ($('#ns_'+key+'_lat')?.value  || '').trim();
      const lonRaw= ($('#ns_'+key+'_lon')?.value  || '').trim();
    ...
    WJ(LS_SETTINGS, st);  // rapporter + shortcuts
    alert('Lagret ✅');
  }

  // ===== Adresse-register (som tidligere)
  ...
  function currentAddresses(){ const cache = (window.Sync?.getCache?.() || {}); return (cache.addresses || []).map(x=>({...x})); }
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    adminLoad();
    $('#admin_save_bins')?.addEventListener('click', adminSave);
