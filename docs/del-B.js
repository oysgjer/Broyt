// Del-B – Under arbeid (én adresse om gangen, store knapper) – med service-krav
(function () {
  ...
  function mapAddr(a) {
    const st = (a.stikkerSeason && a.stikkerSeason.season === S.season) ? a.stikkerSeason : { season:S.season, done:false, doneAt:null };
    const stCount = Number(a.stikkerCount ?? a.stikker_target ?? a.stikkerAntall ?? 0) || 0;
    const status = a.status || (a.done ? 'done' : 'idle');
    return { name:a.name||'Uten navn', group:a.group||'', equipment:Array.isArray(a.equipment)?a.equipment:[], active:a.active!==false,
      done:!!a.done, grusDone:!!a.grusDone, status, startedAt:a.startedAt||null, finishedAt:a.finishedAt||null, notes:a.notes||'',
      stikkerSeason:st, stikkerCount:stCount };
  }
  function filterByJob(list) {
    const job = S.round?.job || 'SNØ';
    if (job === 'GRUS') return list.filter(a => Array.isArray(a.equipment) && a.equipment.includes('stro'));
    return list;
  }
  function renderAddress() {
    const total = S.filtered.length; els.total.textContent = String(total);
    if (!total) { els.idx.textContent='0'; els.name.textContent='Ingen adresser i visning'; els.group.textContent=''; els.equip.textContent='';
      els.stCount.textContent='—'; els.stChk.checked=false; els.status.textContent=''; return; }
    if (S.i<0) S.i=0; if (S.i>total-1) S.i=total-1; els.idx.textContent=String(S.i+1);
    ...
    els.stChk.checked = !!a.stikkerSeason?.done; els.stLbl.textContent = `Stikker satt (${S.season})`;
    const statusTxt = { idle:'Ikke påbegynt', started:'Pågår…', skipped:'Hoppet over', blocked:'Ikke mulig', done:'Ferdig', accident:'Uhell registrert' }[a.status] || '—';
    els.status.textContent = statusTxt;
    els.nav.onclick = () => { const query = [a.name, a.group].filter(Boolean).join(', '); window.open(`https://www.google.com/maps?q=${encodeURIComponent(query)}`, '_blank'); };
  }
  function renderProgress() {
    const list = S.filtered; const doneCount = list.filter(a => a.status==='done').length;
    const pct = list.length ? Math.round((doneCount/list.length)*100) : 0; els.prog.style.width = `${pct}%`;
  }
  function setStatus(a, status) {
    a.status = status; if (status==='started') a.startedAt = a.startedAt || Date.now();
    if (status==='done') a.finishedAt = Date.now(); else a.finishedAt = null; a.done = (status==='done');
  }
  ...
  els.stChk.onchange = (e) => { const a=S.filtered[S.i]; a.stikkerSeason=a.stikkerSeason||{season:S.season,done:false,doneAt:null};
    a.stikkerSeason.season=S.season; a.stikkerSeason.done=!!e.target.checked; a.stikkerSeason.doneAt=e.target.checked?Date.now():null; };
  async function sync() {
    try {
      els.sync.textContent='Henter…';
      const cloud = await getLatest();
      const arr = Array.isArray(cloud) ? cloud : Array.isArray(cloud.addresses) ? cloud.addresses :
      ...
      setMeta('OK'); renderAddress(); renderProgress();
    } catch(e){ console.error(e); els.sync.textContent='Feil'; alert('Synk feilet: '+e.message); }
  }
  async function save() {
    try {
      els.sync.textContent='Lagrer…';
      const payload = { version: window.APP_CFG?.APP_VER || '9.x', round: S.round?.number || 1, updated: Date.now(),
        by: S.round?.driver?.name || 'driver', driver: S.round?.driver || {}, season: S.season, addresses: S.addresses };
      await putPayload(payload); setMeta('Lagret');
    } catch(e){ console.error(e); els.sync.textContent='Feil'; alert('Lagring feilet: '+e.message); }
  }
  els.syncBtn.onclick = sync; els.saveBtn.onclick = save;
  function openFinish(){ $('#b_finish_modal').classList.remove('hidden'); }
  function closeFinish(){ $('#b_finish_modal').classList.add('hidden'); }
  ...
  els.finishSame.onclick = () => {
    ...
    S.round = next || S.round; S.filtered = filterByJob(S.addresses);
    S.i = (S.round?.driver?.direction === 'Motsatt') ? Math.max(0, S.filtered.length - 1) : 0;
  ...
  els.finishClose.onclick = () => {
    const payload = { round: S.round?.number || 0, startedAt: S.round?.startedAt || Date.now(),
      job: S.round?.job || 'SNØ', driver: S.round?.driver?.name || 'driver', createdAt: Date.now() };
    localStorage.setItem(LS_SERVICE_REQ, JSON.stringify(payload));
    closeFinish(); location.hash='#service'; if (typeof window.APP?.go === 'function') window.APP.go('service');
  };
  ...
  function boot() {
    ...
    setMeta('—'); els.hint.textContent = (S.round?.job === 'GRUS') ? 'Viser kun adresser med grus-behov.' : 'Viser alle aktive adresser.';
    sync();
  }
  boot();
})();
